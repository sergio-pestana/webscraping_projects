...
##### RESULTADOS DA MINHA CARTEIIRA TEORICA
# def analise_carteira(mix):
#     ibov = web.get_data_yahoo('^BVSP', period='1d')['Adj Close']
#     ibov_pct = (1+ibov.pct_change()).cumprod().dropna(inplace=True)
    
#     carteira = web.get_data_yahoo(mix, period='1d')['Adj Close']
#     carteira['SALDO'] = carteira.sum(axis=1)
#     carteira_acum = (1+carteira.pct_change()).cumprod().dropna(inplace=True)

#     diff = round(carteira_acum.iloc[-1] - ibov_pct.iloc[-1], 2)

#     sns.set()
#     carteira_acum.plot(figsize=(18,8), label='Carteira Analisada')
#     ibov_pct.plot(label = 'IBOVESPA')
#     plt.legend()

#     return print('Diferença {}\nAcumulado IBOV {}\nAcumulado carteria analisada {}'.format(diff, round(ibov_pct.iloc[-1], 2), round(carteira_acum.iloc[-1], 2)))


# minha_carteira = ['PETR3.SA', 'VALE3.SA', 'B3SA3.SA', 'JBSS3.SA', 'LAME4.SA', 'TIMP3.SA', 'CCRO3.SA', 'TOTS3.SA', 'CYRE3.SA', 'SULA11.SA']
